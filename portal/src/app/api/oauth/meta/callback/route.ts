...
import { exchangeMetaCode, parseOAuthState } from "@/lib/oauth";
...
export async function GET(request: NextRequest) {
  ...
  if (!code || !state) {
    return NextResponse.redirect(new URL("/connect?error=meta_denied", request.url));
  }
  ...
  try {
    ...
    const { encrypted, nonce } = encrypt({ META_ACCESS_TOKEN: longLivedToken });

    const [existing] = await db.select({ id: platformConnections.id }).from(platformConnections)
      .where(and(eq(platformConnections.tenantId, parsed.tenantId), eq(platformConnections.platform, "meta")))
      .limit(1);
    ...
    if (existing) {
      ...
      await db.insert(platformConnections).values({
        tenantId: parsed.tenantId, platform: "meta",
        credentialsEncrypted: encrypted, credentialsNonce: nonce,
      });
    }

    return NextResponse.redirect(new URL("/connect?success=meta", request.url));
  } catch (err) {
    console.error("Meta OAuth callback error:", err);
    return NextResponse.redirect(new URL("/connect?error=meta_exchange", request.url));
  }
}
